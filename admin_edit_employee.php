...
// --- Fetch User Data for the specified UID ---
$user_data = null;
$sql_get_user = "SELECT 
...
if ($stmt_get_user = $mysqli->prepare($sql_get_user)) {
    ...
    if ($stmt_get_user->execute()) {
        ...
        error_log("Error executing user fetch query: " . $stmt_get_user->error);
    ...
    $_SESSION['error_message'] = "Database error retrieving user.";
    header("location: admin_user_management.php");
    exit;
}
...
// If form_data exists in session (meaning validation failed), use it. Otherwise, use fetched user_data.
$form_data = $_SESSION['edit_form_data'] ?? $user_data; 
    ...
    <div class="dashboard">
        ...
        <main class="container dashboard-main">
            <div class="page-header">
                <h1>Edit Employee: <?php echo htmlspecialchars($form_data['fname'] . ' ' . $form_data['lname']); ?></h1>
                                 ...
                                 <?php if (!empty($teams_for_options)): ?>
                                    <?php foreach ($teams_for_options as $team_opt): ?>
                                        <option value="<?php echo htmlspecialchars($team_opt['teamID']); ?>" 
                                                <?php echo (isset($form_data['teamID']) && $form_data['teamID'] == $team_opt['teamID']) ? 'selected' : ''; ?>>
