...
if (!isset($mysqli) || !$mysqli instanceof mysqli) {
     error_log("Database connection object (\$mysqli) not available in admin_add_schedule.php.");
...
if ($mysqli && !$page_error) {
    ...
    if($result_programs_options = $mysqli->query($sql_programs_options)){
        ...
        error_log("Error fetching programs for add schedule options: " . $mysqli->error);
...
<head>
    ...
    <title>EXCEED - Add Schedule Entry</title>
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-actions { margin-top: 2rem; display: flex; justify-content: flex-end; gap: 1rem; border-top: 1px solid var(--border-color); padding-top: 1.5rem; }
    ...
    <div class="dashboard">
        ...
        <main class="container dashboard-main">
            ...
            <div class="card form-container">
                ...
                <form id="addScheduleForm" action="process_add_schedule.php" method="POST" novalidate>
                             ...
                             <?php if (!empty($programs_for_options)): ?>
                                <?php foreach ($programs_for_options as $program_opt): ?>
                                    <option value="<?php echo htmlspecialchars($program_opt['trainingProgramID']); ?>"
                                            <?php echo (isset($form_data['programId']) && $form_data['programId'] == $program_opt['trainingProgramID']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-field-group">
                        ...
                        <input type="date" id="eventDate" name="eventDate" required 
                               value="<?php echo htmlspecialchars($form_data['eventDate'] ?? ''); ?>"
                     ...
                     <div class="form-field-group">
                        ...
                        <input type="time" id="eventTime" name="eventTime" required
                               value="<?php echo htmlspecialchars($form_data['eventTime'] ?? ''); ?>" 
                     ...
                     <div class="form-field-group">
                        ...
                        <select id="status" name="status" required class="<?php echo isset($errors['status']) ? 'is-invalid' : ''; ?>">
                            <option value="Scheduled" <?php echo (isset($form_data['status']) && $form_data['status'] == 'Scheduled') ? 'selected' : (!isset($form_data['status']) ? 'selected' : ''); ?>>Scheduled</option>
                            <option value="Cancelled" <?php echo (isset($form_data['status']) && $form_data['status'] == 'Cancelled') ? 'selected' : ''; ?>>Cancelled</option>
                            </select>
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" class="btn-primary">Add Schedule Entry</button>
                    </div>
                </form>
            </div>
        </main>
