...
if (!isset($mysqli) || !$mysqli instanceof mysqli) {
     error_log("Database connection object (\$mysqli) not available in admin_user_management.php.");
...
// Fetch All Users (Joining to get Team Name and Position if available)
$users = [];
$sql_users = "SELECT 
                ...
                t.teamName,
                t.teamID, 
                'Active' AS status -- Placeholder for actual status logic if added later
              FROM tblUser u
...
if($result_users = $mysqli->query($sql_users)){
    ...
    error_log("Error fetching users for management page: " . $mysqli->error);
    $page_error = "Could not retrieve user data."; // Display generic error
}

// Fetch teams for dropdowns (might be needed on other pages)
$teams_for_options = [];
...
<head>
    ...
    <title>EXCEED - User Management</title>
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-header { margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;} /* Allow wrapping */
        /* END: Core Styles */
        ...
        .filter-controls input[type="text"] { flex-grow: 1; min-width: 200px;} /* Allow search to grow */
        ...
        <main class="container dashboard-main">
            <div class="page-header">
                <h1>User Management</h1>
                 <a href="admin_add_employee.php" class="btn-primary">
                            ...
                            <?php if (!empty($users)): ?>
                                <?php foreach ($users as $user): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($user['teamName'] ?? 'N/A'); ?></td>
                                        <td><?php echo htmlspecialchars($user['position'] ?? 'N/A'); ?></td>
                                        <td><span class="status-badge active"><?php echo htmlspecialchars($user['status']); ?></span></td> 
                                        <td>
                                            <a href="admin_edit_employee.php?uid=<?php echo $user['UID']; ?>" 
