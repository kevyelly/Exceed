...
if (!isset($mysqli) || !$mysqli instanceof mysqli) {
     error_log("Database connection object (\$mysqli) not available in admin_add_team.php.");
     // Set page error instead of dying immediately, so the page structure still loads
     $page_error = "Database configuration error. Cannot load all page data."; 
}

// --- Fetch Programs for Dropdown (Optional) ---
$programs_for_options = [];
// Only attempt to fetch if the connection is valid
if ($mysqli && !$page_error) { 
    ...
    if($result_programs_options = $mysqli->query($sql_programs_options)){
        ...
        error_log("Error fetching programs for add team options: " . $mysqli->error);
        // Set a non-fatal page error, the dropdown will just be empty or show an error
...
<head>
    ...
    <title>EXCEED - Add New Team</title>
    <style>
        ...
        <main class="container dashboard-main">
            ...
            <div class="card form-container">
                ...
                <?php // Display page-level error if connection failed before form processing
                    if ($page_error && !isset($errors['db'])): // Avoid double display if processing also failed
                ?>
                ...
                <form id="addTeamForm" action="process_add_team.php" method="POST" novalidate>
                             ...
                             <?php if (!empty($programs_for_options)): ?>
                                <?php foreach ($programs_for_options as $program_opt): ?>
                                    <option value="<?php echo htmlspecialchars($program_opt['trainingProgramID']); ?>"
                                            <?php // Check form_data first, then default (which is none for add)
                                                $selected_program = $form_data['primaryProgram'] ?? null;
